...
function generateChronologyDraft(fragments: MemoryFragment[]): ChronologyResult {
...
router.post('/', (req, res) => {
  try {
    const { fragments }: { fragments: MemoryFragment[] } = req.body

    if (!fragments || !Array.isArray(fragments)) {
      return res.status(400).json({ error: 'Invalid input: fragments array is required' })
    }

    const result = generateChronologyDraft(fragments)
    return res.json(result)
  } catch (error) {
    console.error('Error generating chronology:', error)
    return res.status(500).json({ error: 'Failed to generate chronology draft' })
  }
})
